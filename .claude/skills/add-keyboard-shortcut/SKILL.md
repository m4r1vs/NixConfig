...
Query the nixos MCP server for home-manager hyprland configuration (e.g. `nix {"action":"search","source":"home-manager","query":"hyprland"}`).
Bind-specific documentation is available in [docs/Hyprland-Binds.md](docs/Hyprland-Binds.md).

## 2. Determine if a script is necessary
...
If the action is probably best done by hyprland itself, look at [docs/Hyprland-Dispatchers.md](docs/Hyprland-Dispatchers.md) for a list of hyprland dispatchers.

## 3. Implement keybind

Now edit `home-manager/modules/hyprland.nix` to add the keybind based on the user's inputs. Remember to always add a description.
