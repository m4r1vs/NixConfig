{pkgs, ...}: {
  cpu-info =
    ...
    ''
      ...
      sensors_json=$(${pkgs.lm_sensors}/bin/sensors -j 2>/dev/null)
      ...
      frequency=$(${pkgs.perl}/bin/perl -ne 'BEGIN { $sum = 0; $count = 0 } if (/cpu MHz\s+:\s+([\d.]+)/) { $sum += $1; $count++ } END { if ($count > 0) { printf "%.2f\n", $sum / $count } else { print "NaN\n" } }' /proc/cpuinfo)
